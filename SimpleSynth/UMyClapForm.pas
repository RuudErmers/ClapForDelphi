...
uses
  ...
  Vcl.Controls, Vcl.Forms, Vcl.Dialogs, Vcl.StdCtrls,UMyClap,UPianoKeyboard;
...
var
  FormMyClap: TFormMyClap;

implementation

{$R *.dfm}

procedure TFormMyClap.FormCreate(Sender: TObject);
begin
...
procedure TFormMyClap.ScrollBar1Change(Sender: TObject);
VAR isb:integer;
begin
  for isb:=0 to 2 do
...
procedure TFormMyClap.Button1Click(Sender: TObject);
begin
  if assigned(HostPrgmChange) then
    HostPrgmChange(1);
end;

procedure TFormMyClap.CBOnKeyEvent(Sender:TObject;key:integer;_on,infinite:boolean);
begin
  if assigned(HostKeyEvent) then
    HostKeyEvent(key,_on);
end;

procedure TFormMyClap.SetKey(key: integer; _on: boolean);
begin
  Fkeyboard.SetKeyPressed(key,_on);
end;

procedure TFormMyClap.SetProgram(prgm: integer);
begin
  Label2.Caption:='Program:'+prgm.toString;
end;

procedure TFormMyClap.UpdateEditorParameter(index:integer;value: double);
VAR isb:integer;
begin
  for isb:=0 to 2 do
